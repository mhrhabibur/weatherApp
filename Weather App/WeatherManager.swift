...
class WeatherManager {
    ...
    func fetchDataAutomatically(latitude: Double, longitude: Double, completion: @escaping (WeatherMapping?, Error?) -> Void) {
        
       if let url = URL(string: "https://api.openweathermap.org/data/2.5/weather?units=metric&lat=\(latitude)&lon=\(longitude)&appid=1f462dda351a1f36c3d8b29280ef0fdb")
        {
        URLSession.shared.dataTask(with: url) { data, response, error in
            if error != nil {
                return
